...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dropout(0.3), #Minina o pesos das perdas
...
def model_predict(data_asarry):
    data_asframe = pd.DataFrame(data_asarry, columns=X_train.columns)
    data_asarry = save_scalers(data_asframe,['tempoprofissao', 'renda', 'idade', 'dependentes', 'valorsolicitado', 'valortotalbem', 'proporcaosolicitadototal'])
